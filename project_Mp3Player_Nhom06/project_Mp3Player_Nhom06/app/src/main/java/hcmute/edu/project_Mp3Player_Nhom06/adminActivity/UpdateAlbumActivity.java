...
public class UpdateAlbumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_saveAlbum.setText("Save change");
        btn_addImage.setText("Update image");
        Intent intentGetAlbum = getIntent();
        albumId = intentGetAlbum.getStringExtra("AlbumId");
        albumInfo = (Album) intentGetAlbum.getSerializableExtra("AlbumInfo");
        ...
        editText_albumName.setText(albumInfo.getName());
        editText_albumDescription.setText(albumInfo.getDescription());
        Glide.with(imageView_image.getContext())
                .load(albumInfo.getImage())
    ...
    private void saveImgToStorage() {
        if(imageUri != null) {
            ...
            SaveAlbumToFireStore(albumInfo.getImage());
        }
    }

    private void deleteOldImage() {
        StorageReference storageReference = storage.getReferenceFromUrl(albumInfo.getImage());
        ...
        countQuery.get(AggregateSource.SERVER).addOnCompleteListener(new OnCompleteListener<AggregateQuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<AggregateQuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    singer.put("relatedSongs", albumInfo.getRelatedSongs());
                    fStore.collection("albums").document(albumId)
                            .set(singer)
                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void unused) {
                                    Toast.makeText(UpdateAlbumActivity.this, "Album updated", Toast.LENGTH_SHORT).show();
                            ...
                            .addOnFailureListener(new OnFailureListener() {
                                @Override
                                public void onFailure(@NonNull Exception e) {
                                    Toast.makeText(UpdateAlbumActivity.this, "Please check again", Toast.LENGTH_SHORT).show();
                                }
                            });
